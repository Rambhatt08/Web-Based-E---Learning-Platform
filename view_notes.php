...
// 4. Build SQL
$sql = "SELECT * FROM notes WHERE branch = '$branch_code' AND year_level = '$year_level'";

if (!empty($search_query)) {
    $safe_search = mysqli_real_escape_string($conn, $search_query);
    $sql .= " AND (title LIKE '%$safe_search%' OR subject LIKE '%$safe_search%')";
}
...
<head>
    ...
    <title>Notes: <?php echo $branch_code; ?> - Smart Learn</title>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <link rel="stylesheet" href="style.css">
    
    <style>
        /* --- 1. SEARCH BAR CSS (FIXED) --- */
        .search-container {
            /* Align Left and Below Banner */
        ...
        /* --- 2. PURPLE HERO THEME CSS --- */
        .hero-header {
            background: linear-gradient(135deg, #a4508b 0%, #5f0a87 74%);
            color: white;
            padding: 40px 20px; /* Reduced padding slightly */
            text-align: center;
            /* Removed bottom margin so search bar sits tighter if needed */
            margin-bottom: 0; 
        ...
        .notes-grid { 
            ...
            margin-top: 20px; 
            padding-bottom: 50px;
            padding-left: 30px; /* Align grid with search bar */
            padding-right: 30px;
        }
        ...
        .btn-read:hover { background: #2980b9; }
...
.search-btn svg {
    width: 20px;
    height: 20px;
    stroke: white; /* This makes the lines WHITE */
    stroke-width: 2.5; /* Thickness of the lines */
    fill: none;    /* No background color inside the glass */
}
...
<div class="dashboard-container">
    ...
    <aside class="sidebar">
        <ul>
            ...
            <li><a href="#" class="active"><i class="fas fa-book"></i> Notes</a></li>
    ...
    <main class="dash-content">
        ...
        <div class="hero-header">
             <?php 
                if($branch_code) {
                    echo '<h1 class="hero-title">' . strtoupper($year_level) . '</h1>';
                    echo '<p class="hero-subtitle">Make learning effective - ' . $branch_name . '</p>';
                } else {
                    echo '<h1 class="hero-title">LIBRARY</h1>';
                    echo '<p class="hero-subtitle">Select a Branch from the Menu</p>';
                }
             ?>
        </div>

        <div class="search-container">
            <form action="view_notes.php" method="GET">
        ...
        <div class="notes-grid">
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    $thumb_path = !empty($row['thumbnail']) ? $row['thumbnail'] : 'uploads/thumbnails/default.png';
                    $pdf_link = str_replace(" ", "%20", $row['file_path']);

                    echo '
                    <div class="note-card">
                        <a href="'.$pdf_link.'" target="_blank" class="card-img-wrapper">
                            <img src="'.$thumb_path.'" class="card-img" alt="Thumbnail">
                        </a>
                        <div class="card-body">
                            <a href="'.$pdf_link.'" target="_blank" style="text-decoration:none;">
                                <h4 class="card-title">'.$row['title'].'</h4>
                            </a>
                            <p class="card-subject"><i class="fas fa-book"></i> '.$row['subject'].'</p>
                            <a href="'.$pdf_link.'" target="_blank" class="btn-read">
                ...
                if(!empty($search_query)) {
                    ...
                    echo '<div class="no-notes">
                        <i class="fas fa-folder-open" style="font-size: 50px; margin-bottom: 20px; color: #ccc;"></i><br>
                        No notes uploaded yet.
                      </div>';
                } else {
                    echo '<p style="text-align:center;">Select Branch and Year.</p>';
                }
            }
            ?>
        </div>
